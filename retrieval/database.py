...
class DataBase:
    ...
    def add_table_via_df(self, table_name: str, df: pd.DataFrame):
        conn = sqlite3.connect(self.db_file)
        df.to_sql(table_name, con=conn, if_exists='replace', index=False)
        conn.close()
        print(f"{df.shape[0]} stored into {table_name} in {self.db_file}")

    def add_table_via_csv(self, csv_file: str, table_name):
        df = pd.read_csv(csv_file)
        df['id'] = df.index
        conn = sqlite3.connect(self.db_file)
        df.to_sql(table_name, con=conn, if_exists='replace', index=False)
    ...
    def get_texts_and_metadata(self, table_name: str, texts_col_name: str) -> list[str]:
        conn = sqlite3.connect(self.db_file)
        df = pd.read_sql(f"SELECT * FROM {table_name}", con=conn)
        texts = df[f"{texts_col_name}"].tolist()
        return texts, df
    
    def get_rows(self, ids: list[id], table_name: str) -> pd.DataFrame:
        conn = sqlite3.connect(self.db_file)
        df = pd.read_sql(f"SELECT * FROM {table_name}", con=conn)
